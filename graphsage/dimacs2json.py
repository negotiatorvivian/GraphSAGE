...
class CompactSatDimacs:
    ...
    def __init__(self, dimacs_file, ans_file = None):
        ...
        if self.content is None:
            ...
            ind = np.where(np.sum(np.abs(self._clause_mat), 1) > 0)[0]
            self._clause_mat = self._clause_mat[ind, :]
            self.pairs = self.generate_relations()
    ...
    def to_json_promote(self, base = 0):
        try:
            ...
            for i in range(var_num):
                ...
                node = {
                    "test": True if random.random() > 0.8 else False,
                    "id": i + base,
                    "features": np.abs(self._clause_mat_new[:, i]).tolist(),
                    "val": True if random.random() > 0.9 else False,
                    "label": [1, 0] if self.content[i] < 0 else [0, 1]
                }
        ...
        return nodes, links, label, var_num + base
    ...
    def generate_relations(self):
        ...
        for index in range(self._clause_mat_new.shape[1]):
            ...
            pos_pairs = [np.where(self._clause_mat_new[i, :] != 0)[0].tolist() for i in func_pos_index]
            neg_pairs = [np.where(self._clause_mat_new[i, :] != 0)[0].tolist() for i in func_neg_index]
            for item in pos_pairs:
                item.remove(index)
                links[index].append(item)
    ...
    try:
        ...
        if answer_dir is None:
            ...
            for i, file in enumerate(file_list):
                ...
                if bc.flag is False:
                    continue
                node, link, label, base = bc.to_json_promote(base)
